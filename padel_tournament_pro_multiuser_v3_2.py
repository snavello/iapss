# padel_tournament_pro_multiuser_v3_2.py
# Adds an inline SVG logo (top-left) + keeps all v3.1 features.
...
st.set_page_config(page_title="Torneo de Pádel — Multiusuario v3.2", layout="wide")

# ----------------------------
# Inline brand (chosen option #1)
# ----------------------------
PRIMARY_BLUE = "#0D47A1"   # deep blue
LIME_GREEN  = "#AEEA00"    # padel ball green
DARK_BLUE   = "#082D63"

def brand_svg(width_px: int = 220) -> str:
    svg = f'''<svg xmlns="http://www.w3.org/2000/svg" width="{width_px}" viewBox="0 0 660 200" role="img" aria-label="iAPPs PADEL TOURNAMENT">
  <defs>
...
def tournament_manager(user: Dict[str, Any], tid: str):
    ...
    with tab_cfg:
        ...
        if st.session_state.pdf_fixture_bytes or st.session_state.pdf_playoffs_bytes:
            ...
            if st.session_state.pdf_fixture_bytes:
                st.download_button("⬇️ Fixture (PDF)",
            ...
            if st.session_state.pdf_playoffs_bytes:
                st.download_button("⬇️ Playoffs (PDF)",
    ...
    with tab_persist:
        ...
        with c3:
            st.download_button("⬇️ Descargar estado (JSON)", data=json.dumps(state, ensure_ascii=False, indent=2).encode("utf-8"), file_name=f"{tid}.json", mime="application/json")
        with c4:
...
st.caption("v3.2 — Logo iAPPs/PADEL/TOURNAMENT (top-izq) + descargas PDF robustas + KO por sets/GP + autosave.")
init_app()
