...
/* Inputs angostos cuando están dentro de un contenedor con clase .thin */
...
# ====== Playoffs helpers (con IDs estables) ======
...
def ensure_match_ids(matches: List[Dict[str,Any]]):
    """Añade 'mid' estable a cada partido si no existe."""
    for m in matches:
        if not m.get("mid"):
            rn = m.get("round","R")
            lb = m.get("label","M")
...
def build_initial_ko(qualified: List[Tuple[str,int,str]], best_of_fmt: str="best_of_3") -> List[Dict[str,Any]]:
    """Crea los partidos iniciales con mid estable. best_of_fmt es 'one_set'|'best_of_3'|'best_of_5'."""
    N = len(qualified)
    if N == 0: return []
    ...
    while i < len(names):
        ...
        m["mid"] = _mid_for(start_round, lab, a, b or "BYE")
...
def pairs_to_matches(pairs: List[Tuple[str, Optional[str]]], round_name: str, best_of_fmt: str="best_of_3") -> List[Dict[str,Any]]:
    ...
    for i,(a,b) in enumerate(pairs, start=1):
        ...
        m["mid"] = _mid_for(round_name, lab, a, b or "BYE")
        out.append(m)
    return out
...
def create_groups_seeded(pairs: List[str], seeded_labels: List[str], num_groups: int, top_per_zone: int, seed: int) -> List[List[str]]:
    ...
    groups = [[] for _ in range(num_groups)]
    # 1 cabeza de serie por grupo
    for i, s in enumerate(seeded[:num_groups]):
        groups[i].append(s)
    # mínimo por zona = top_per_zone
    min_per_zone = max(1, int(top_per_zone))
    total = len(pairs)
    desired_min_total = num_groups * min_per_zone
    gi = 0
...
def tournament_manager(user: Dict[str, Any], tid: str):
    ...
    tab_cfg, tab_pairs, tab_results, tab_tables, tab_ko = st.tabs(
        ["⚙️ Configuración", "👥 Parejas", "📝 Resultados", "📊 Tablas", "🏁 Playoffs"]
    )
    ...
    with tab_cfg:
                ...
                if len(pairs) < cfg["num_zones"]:
                    ...
                    if cfg.get("use_seeds", False):
                        ...
                        if len(seeded) != int(cfg["num_zones"]):
                            st.error(f"Seleccioná exactamente {int(cfg['num_zones'])} cabezas de serie antes de sortear.")
                            st.stop()
        ...
        with c3p:
            ...
            st.download_button(
                ...
                data=payload,
                file_name=fname, mime="application/json", key=f"dl_state_json_{tid}"
            )
        with c4p:
            ...
            if up is not None:
                ...
                if st.button("Confirmar restauración", key=f"confirm_restore_{tid}", type="primary"):
                    try:
                        ...
                        st.session_state.last_hash = compute_state_hash(new_state)
                        st.session_state.autosave_last_ts = time.time()
    ...
    with tab_pairs:
        ...
        with colL:
            ...
            with st.form(f"add_pair_form_{tid}", clear_on_submit=True):
                ...
                with c1:
                    st.text_input("N°", value=(str(next_n) if next_n else "—"), disabled=True)
                with c2:
            ...
            if subm:
                ...
                if not p1c or not p2c:
                    ...
                    st.rerun()
            if next_n is None:
                st.warning(f"Se alcanzó el máximo de parejas ({max_pairs}).")
        ...
        if st.button("🎲 Sortear zonas", key=f"sorteo_pairs_{tid}"):
            ...
            if len(pairs) < cfg["num_zones"]:
                ...
                if cfg.get("use_seeds", False):
                    seeded = state.get("seeded_pairs", [])
                    if len(seeded) != int(cfg["num_zones"]):
                        st.error(f"Debes marcar {int(cfg['num_zones'])} cabezas de serie antes del sorteo.")
                        st.stop()
                    groups = create_groups_seeded(pairs, seeded, int(cfg["num_zones"]), int(cfg["top_per_zone"]), int(cfg["seed"]))
                else:
                    groups = create_groups_unseeded(pairs, int(cfg["num_zones"]), int(cfg["top_per_zone"]), int(cfg["seed"]))
                ...
                with st.container(border=True):
                    ...
                    if st.button("Guardar partido", key=f"sv_{tid}_{idx}"):
                        ...
                        if stats["sets1"] == stats["sets2"]:
                            ...
                            save_tournament(tid, state)
                            # sincroniza hash/ts para evitar autosave inmediato
                            st.session_state.last_hash = compute_state_hash(state)
                            st.session_state.autosave_last_ts = time.time()
    ...
    def render_playoff_match(tid_: str, round_name: str, match: dict, tourn_state: dict):
        # mid estable
        mid = match.get("mid")
        if not mid:
        ...
        # best_of según configuración del match
        ...
        cN,_,_ = st.columns([1,1,1])
        with cN:
            ns_key = widget_key(tid_, mid, "ko_ns")
            n_sets = st.number_input(
                "Sets jugados", min_value=n_min, max_value=n_max,
                value=min(max(len(cur_sets), n_min), n_max),
                key=ns_key
        ...
        for si in range(n_sets):
            cA,cB,_ = st.columns([1,1,1])
            with cA:
                s1_key = widget_key(tid_, mid, f"ko_s{si+1}_a")
                s1_val = int(cur_sets[si]["s1"]) if si<len(cur_sets) and "s1" in cur_sets[si] else 0
                s1 = st.number_input(f"Set {si+1} — {match['a']}", 0, 20, s1_val, key=s1_key)
            with cB:
                s2_key = widget_key(tid_, mid, f"ko_s{si+1}_b")
                s2_val = int(cur_sets[si]["s2"]) if si<len(cur_sets) and "s2" in cur_sets[si] else 0
                s2 = st.number_input(f"Set {si+1} — {match['b']}", 0, 20, s2_val, key=s2_key)
            new_sets.append({"s1":int(s1), "s2":int(s2)})
        ...
        gC,gD,_ = st.columns([1,1,1])
        with gC:
            g1_key = widget_key(tid_, mid, "ko_g1")
            g1 = st.number_input(f"Puntos de oro {match['a']}", 0, 200, int(match.get("goldenA",0)), key=g1_key)
        with gD:
            g2_key = widget_key(tid_, mid, "ko_g2")
            g2 = st.number_input(f"Puntos de oro {match['b']}", 0, 200, int(match.get("goldenB",0)), key=g2_key)
        ...
        if st.button("Guardar partido KO", key=save_key):
            ...
            match["sets"] = new_sets
            match["goldenA"] = int(g1)
            match["goldenB"] = int(g2)
    ...
    def render_playoff_round(tid_: str, round_name: str, matches: list, tourn_state: dict):
        for m in matches:
            with st.container(border=True):
                render_playoff_match(tid_, round_name, m, tourn_state)
    ...
    with tab_ko:
        ...
        if not state.get("groups") or not state.get("results"):
            ...
            if not all_complete:
                ...
                # Asegurar 'mid' en todos los partidos existentes
                ensure_match_ids(state["ko"]["matches"])
                ...
                for rname in ["QF","SF"]:
                    ...
                    if all_done:
                        ...
                        if next_rname:
                            ...
                            # limpiar ronda siguiente previa
                ...
                # Si existe FINAL y tiene ganador, destacarlo
    ...
    # ===== Autosave con cooldown =====
    current_hash = compute_state_hash(state)
    now_ts = time.time()
    if st.session_state.autosave and current_hash != st.session_state.last_hash:
        if now_ts - st.session_state.autosave_last_ts >= 4.0:  # cooldown 4s
            save_tournament(tid, state)
            st.session_state.last_hash = current_hash
            st.session_state.autosave_last_ts = now_ts
