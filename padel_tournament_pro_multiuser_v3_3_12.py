# padel_tournament_pro_multiuser_v3_3_12.py
# Cambios v3.3.12:
# 1) Barra superior fija: logo (máx 20% ancho), datos de usuario/rol, botón salir; no tapa contenido; aplica también a Super Admin y Viewer.
# 2) Super Admin: tras crear usuario aparece inmediatamente en la lista editable (y se limpia el formulario).
# 3) Parejas: al grabar una pareja se blanquean campos de Jugador 1/2; botón "SORTEAR ZONAS" también aquí y solo activo si se alcanzó el máximo.
...
st.set_page_config(page_title="Torneo de Pádel — Multiusuario v3.3.12", layout="wide")
...
# ====== Config App (global) ======
DEFAULT_APP_CONFIG = {
    "app_logo_url": ""  # URL pública del logo de la app. Si vacío, se usa SVG embebido.
}
...
def inject_global_layout(user_info_text: str):
    """Crea una barra superior fija con logo a la izquierda (máx 20% ancho) y datos usuario/rol a la derecha.
       Además hace sticky las tabs para que queden visibles arriba y no tapen contenido.
    """
    ...
    <style>
      ...
      .content-offset {{
        padding-top: 76px; /* altura aprox. de la barra fija */
      }}
      /* Tabs sticky */
      ...
      /* Zebra striping y header oscuro para tablas renderizadas con to_html */
      .dark-header th {{
        background-color: #2f3b52 !important; color: #fff !important;
      }}
      ...
      /* Ocultar decoración html cruda accidental */
    ...
    # Crea un contenedor con offset para no quedar tapado
...
def init_session():
    ...
    # campos de entrada que queremos limpiar
    st.session_state.setdefault("last_pair_add_ts", 0)
    st.session_state.setdefault("login_ok", False)
...
def super_admin_panel():
    ...
    with st.expander("🎨 Apariencia (Logo global de la app)", expanded=True):
        app_cfg = load_app_config()
        url = st.text_input("URL pública del logotipo (no por torneo)", value=app_cfg.get("app_logo_url","")).strip()
    ...
    # Lista editable de usuarios (actualiza inmediatamente)
...
def tournament_manager(user: Dict[str, Any], tid: str):
    ...
    with tab_cfg:
        ...
        with cB:
            # También disponible en pestaña Parejas, aquí siempre activo si hay suficientes parejas
...
def init_app():
    ...
    if mode=="public" and _tid:
        ...
        st.caption("iAPPs Pádel — v3.3.12")
        return
